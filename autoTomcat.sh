...
if [ $IS_EXIST != 0 ]; then
	if [ $ENABLE_SII -eq 1 ]; then
		...
		if [ $isRunningTomcat -gt 2 ]; then	
			...
			sshpass -p $PASSWORD_SI ssh -o StrictHostKeyChecking=no $USER_NAME_SI@$HOST_SI "cd $TOMCAT_PATH_SI && ./bin/catalina.sh stop"
		else
			source ./deployToTomcat.sh $USER_NAME_SI $PASSWORD_SI $HOST_SI $TOMCAT_NAME_SI $HTTP_PORT_SI $TOMCAT_PATH_SI
			sshpass -p $PASSWORD_SII ssh -o StrictHostKeyChecking=no $USER_NAME_SII@$HOST_SII "cd $TOMCAT_PATH_SII && ./bin/catalina.sh stop"
		fi
